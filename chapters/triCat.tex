    ...
    \begin{proof}
        ...
        \begin{itemize}
            ...
            \begin{center}
                ...
                \end{tikzcd}
            \end{center}
            \todo{fiks i morgen} By the Ore condition we can create two new maps from $\widetilde{w'}$ and $\widetilde{w''}$. Since both morphisms are assumed to be in $S$, then we get that both $\widetilde{w'}$ and $\widetilde{w''}$ are in $S$. The following diagram then shows that $fs^{-1}\sim hu^{-1}$. A simple diagram chase shows that it is commutative.
            \begin{center}
                \begin{tikzcd}
