...
% Flytt denne til introduksjonen!!!
    % In literature one are able to find two different types of triangulated categories, they are called algebraic and topological. How these categories differ is however quite subtle.

% What am I going to talk about in this chapter?
    % What a triangulated category is
    % Homotopies on triangles
    % Localization of categories, then fitting triangulated categories into this framework
    % An embedding theorem?

    \todo[color = pink]{Revider innledning til kapittelet} This chapter aims to explain the concept of a triangulated category. The themes to be discussed in this chapter are among the axioms of triangulated categories, their elementary properties, how these categories may be localized and an embedding into an abelian category.
...
\section{Definition and First Properties}
    ...
    \begin{proof}
        ...
        \begin{center}
            ...
            \end{tikzcd}
        \end{center}
        (1) is constructed by applying TR2 twice, while (2) is constructed with TR1 and the applying TR2 twice. Observe that there is a commutative square between the triangles, allowing for TR3 to make a morphism of triangles.
        \begin{center}
            \begin{tikzcd}
...
\section{Mapping Cones, Homotopies and Contractibility}
    ...
    Up until now the Octahedron axiom have not yet been used once, other than for proving TR3. Only by assuming TR1, TR2 and TR3 all of the results from the previous section follows. This is what will motivate the next definition.
...
\section{Calculus of Fractions and the Verdier Quotient}
    ...
    \todo[color = red]{Finn ut hva jeg skal gjøre med disse kildene} More details can be found in \cite{zisman} and \cite{weibel}.
