...
\section{Idempotent Completeness and Krull-Schmidt Categories}
    ...
    This section is based of the ideas from \cite{buhler}, \cite{Kra12} and \cite{Rei95}. \todo{Omformuler} At first the ideas of idempotent complete categories will be introduced, the we will look at a weakening and a strengthening of this condition, known as weak idempotent complete and Krull-Schmidt.
    ...
    \begin{proof}
        ...
        h is split-mono as $hih = (id_B-fg)h=h-fgh=h$. As h is mono from being a kernel, it follows that $ih=id_C$. \todo[color = pink]{Finnes det en bedre formulering her?} $B\simeq A\oplus C$ as $id_B -fg = hi \iff id_B = fg + hi$. This in turn implies that $i$ is the cokernel of $f$.
    \end{proof}
...
\section{Admissable Morphisms, Homology and Long Exact Sequences}
    ...
    \begin{proof}
        ...
        \begin{center}
            ...
            \end{tikzcd}
        \end{center}
        By assumption $ip=jq$, thus $jq\circ Ker(p)=0$. $q\circ Ker(p)=0$ as $j$ is mono, thus there exists a morphism $\phi:I\rightarrow I'$ uniquely such that $q=\phi p$. Now $ip=jq=j\phi p$, and as $p$ is epi it follows that $i=j\phi$. \todo[color = pink]{Er dette ordet stavet riktig?} Reiterating the argument, but with $Ker(q)$ instead, there exists a $\phi '$ uniquely such that $p = \phi 'q$ and $j=i\phi '$. Thus $i=j\phi = i\phi '\phi$, and since $i$ is mono it follows that $id_I=\phi '\phi$; dually $Id_{I'}=\phi\phi '$.
    \end{proof}
...
\section{The Derived Category/Derived Functors}  
    ...
    \todo{Merkelig avsnitt} Given some conditions on the category $(\mathcal{A},\mathcal{E})$ one is able to get a nice description of the derived category. That is we want the category of acyclic chain complexes to be closed under isomorphisms. The following proposition tells us whener this is true.
