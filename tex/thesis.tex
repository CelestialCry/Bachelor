...
\begin{document}
    ...
    \section{Exact Categories}
        ...
        \subsection{The Homotopy Category}
            ...
            The next example of a triangulated category is something which is referred to as \emph{the} triangulated category. The homotopy category is the category which is the prototype for every triangulated category. It can be defined with the following exact category.

            \begin{definition}
                Define the category of cochains
            \end{definition}

            \begin{remark}
                Give a remark that if the base category is abelian then the category of cochains is abelian as well.
            \end{remark}

            Write something about the exact structure which we can append on the category of cochains.

            \begin{prop}
                Show that this gives a Frobenius category. I will probably the need the notion of cones here, idk.
            \end{prop}
            ...
            \begin{prop}
                Show that the quotient defined by the exact structure is the same as the quotient defined by homotopies.
            \end{prop}
            ...
            [Give a more explicit description of the triangle maps. I can maybe use the map defined in Steffens notes.]

        \subsection{Admissable Morphisms, Homology and Long Exact Sequences}
            
            Need to define exact sequences, maybe prove 5 lemma and snake lemma, idk. With exact sequences I am going to descrive the subcategory of acyclic cochains. Should I deduce derived categories here. Probably write something about when the different versions of derived categories exists.

        \subsection{The Derived Category/Derived Functors}

            Goal is to deduce the derived category via the Verdier quotient. Should I maybe write something about derived functors here? Maybe if I find time.
    ...
    \section{Auslander-Reiten Triangles}
        ...
        \subsection{Krull-Schmidt Categories}
            ...
            Define and prove basic results for Krull-Schmidt categories. See your own OneNote.
